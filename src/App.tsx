...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col font-sans antialiased text-[#1A1C1E]">
      
      {/* 1. Header (Hidden during browser Print) */}
      <header className="bg-white border-b border-gray-200 h-14 flex items-center justify-between px-4 sticky top-0 z-30 shadow-sm print:hidden">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2.5">
            <span className="w-8 h-8 rounded-lg bg-amber-500 text-slate-950 font-black text-sm flex items-center justify-center shadow-sm shrink-0 border border-amber-600/30">
              PK
            </span>
            <div className="bg-amber-400 text-slate-950 px-3 py-1 rounded-lg border border-amber-500/40 shadow-sm">
              <h1 className="text-sm sm:text-base font-black tracking-wide leading-none uppercase text-slate-950">
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 pl-3">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-bold text-slate-900 leading-none">{user.nama_lengkap}</p>
              <p className="text-[9px] text-slate-400 font-semibold uppercase mt-0.5">{user.role}</p>
            </div>
            <span className="w-7 h-7 rounded-full bg-amber-100 border border-amber-300 text-amber-900 font-extrabold text-[10px] flex items-center justify-center uppercase shadow-sm">
              {user.nama_lengkap.slice(0, 2)}
            </span>
            <button
              onClick={handleLogout}
              id="btn-top-logout"
              className="ml-1 flex items-center space-x-1 px-2.5 py-1 rounded-lg text-xs font-extrabold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 transition-all cursor-pointer shadow-2xs"
      ...
      {isTrialUser(user) && (
        <div className="bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 text-slate-950 px-4 py-2 font-sans border-b border-amber-800 flex flex-col sm:flex-row items-center justify-between gap-2 shadow-sm shrink-0 print:hidden">
          <div className="flex items-center space-x-2 text-xs font-extrabold">
            <span className="bg-slate-950 text-amber-300 font-black px-2 py-0.5 rounded text-[10px] tracking-wider uppercase">
              AKUN TRIAL (3 HARI)
            </span>
            <span>
              Sisa Waktu Coba: <strong className="text-white underline font-black">{getTrialRemainingTime(user).formatted}</strong> — Seluruh dokumen cetak diberi watermark TRIAL.
            </span>
          </div>
          <div className="flex items-center space-x-2 text-xs">
            <span className="hidden md:inline text-[11px] font-black text-slate-950">
              Lisensi Full: {FULL_LICENSE_PRICE}
            </span>
            <button
              onClick={() => setShowActivationModal(true)}
              className="bg-slate-950 hover:bg-slate-900 text-amber-300 font-black text-[11px] px-3 py-1 rounded-lg transition-all border border-amber-400/30 shadow-xs flex items-center space-x-1 cursor-pointer"
            >
              <KeyRound className="w-3.5 h-3.5 text-amber-400" />
              <span>Aktivasi Kode Lisensi FULL</span>
            </button>
            <button
              onClick={handleLogout}
              className="bg-red-900 hover:bg-red-950 text-red-100 font-bold text-[11px] px-2.5 py-1 rounded-lg transition-all border border-red-700/50 flex items-center space-x-1 cursor-pointer"
